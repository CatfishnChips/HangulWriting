 ...
 public class GestureIO
    {
        ...
        public static Gesture ReadMultiGestureFromXML(string xml) {
			...
			try {
				...
				xmlReader = new XmlTextReader(new StringReader(xml));
				gesture = ReadMultiGesture(xmlReader);
		...
		/// <param name="fileName"></param>
		/// <returns></returns>
		public static Gesture ReadMultiGestureFromFile(string fileName) {
		...
		private static Gesture ReadMultiGesture(XmlTextReader xmlReader)
        {
        ...
        /// Writes a multistroke gesture to an XML file
        /// </summary>
        public static void WriteMultiGesture(PDollarGestureRecognizer.Point[] points, string gestureName, string fileName)
        {
            using (StreamWriter sw = new StreamWriter(fileName))
            {
                sw.WriteLine("<?xml version=\"1.0\" encoding=\"utf-8\" standalone=\"yes\"?>");
